...
const inputClass = (hasError: boolean) => cn(
  "rounded-md border bg-background px-3 py-2 text-sm outline-none transition-shadow w-full",
  "focus:ring-2 focus:ring-ring placeholder:text-muted-foreground",
  hasError && "border-destructive focus:ring-destructive/40"
)

export function LoginPage() {
  const { signInWithEmail, signInWithGoogle, signUpWithEmail, resetPassword } = useAuthStore()
        ...
        <div className="w-full max-w-sm">
          <div className="flex flex-col items-center mb-8">
            ...
            <p className="text-sm text-muted-foreground mt-1">
              {mode === "signup" ? "Create your account" : "Sign in to access your documents"}
            </p>
          </div>
            ...
            <div className="flex flex-col gap-3">
              {resetSent ? (
                <div className="flex flex-col items-center gap-3 py-2 text-center">
                  ...
                  <Button variant="outline" className="mt-2 w-full" onClick={() => { switchMode("signin"); setResetSent(false) }}>
                    Back to sign in
                  </Button>
                </div>
              ) : (
                <form onSubmit={handleResetPassword} className="flex flex-col gap-3">
                  ...
                  <button type="button" className="text-xs text-muted-foreground hover:text-foreground transition-colors" onClick={() => switchMode("signin")}>
                    Back to sign in
                  </button>
                </form>
              )}
              ...
              <p className="text-center text-xs text-muted-foreground">
                {mode === "signin" ? (
                  <>Don't have an account?{" "}
                    <button type="button" className="text-foreground hover:underline" onClick={() => switchMode("signup")}>Sign up</button>
                  </>
                ) : (
                  <>Already have an account?{" "}
                    <button type="button" className="text-foreground hover:underline" onClick={() => switchMode("signin")}>Sign in</button>
                  </>
                )}
              </p>
            </div>
